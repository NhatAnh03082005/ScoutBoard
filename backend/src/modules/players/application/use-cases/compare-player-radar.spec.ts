...
export function getRadarMetrics(
  ...
  switch (profile) {
    case 'GK': {
      ...
      const penSavePct = penFaced > 0 ? (penSaved / penFaced) * 100 : (stat.penaltySavePercentage ?? null);
      ...
      return [
        ...
        { key: 'penaltyStopping', label: 'PENALTY STOPPING', value: penSavePct !== null ? normalizeMetric(penSavePct, 0, 100) : 0, rawValue: `${penSavePct ?? 0}%` },
      ];
    }
    case 'DEF': {
      return [
        ...
        { key: 'duelsWon', label: 'DUEL ABILITY', value: normalizeMetric(duelsP90, 0, 7.0), rawValue: `${duelsP90.toFixed(2)}/90` },
        { key: 'passAccuracy', label: 'PASS ACCURACY', value: normalizeMetric(passAcc, 60, 95), rawValue: `${passAcc.toFixed(1)}%` },
        { key: 'buildUp', label: 'BUILD-UP', value: normalizeMetric(passesP90, 0, 75), rawValue: `${passesP90.toFixed(1)}/90` },
      ];
    }
    case 'CDM': {
      return [
        { key: 'tackles', label: 'TACKLING', value: normalizeMetric(tacklesP90, 0, 4.0), rawValue: `${tacklesP90.toFixed(2)}/90` },
    ...
    case 'LM_RM': {
      return [
        { key: 'tackles', label: 'TACKLING', value: normalizeMetric(tacklesP90, 0, 3.0), rawValue: `${tacklesP90.toFixed(2)}/90` },
        { key: 'interceptions', label: 'INTERCEPTIONS', value: normalizeMetric(intP90, 0, 2.2), rawValue: `${intP90.toFixed(2)}/90` },
        { key: 'passAccuracy', label: 'PASS ACCURACY', value: normalizeMetric(passAcc, 65, 92), rawValue: `${passAcc.toFixed(1)}%` },
    ...
    default: {
      return [
        ...
        { key: 'onTarget', label: 'ON TARGET', value: normalizeMetric(shotsOnTargetP90, 0, 2.0), rawValue: `${shotsOnTargetP90.toFixed(2)}/90` },
        { key: 'creativity', label: 'CHANCE CREATION', value: normalizeMetric(keyPassesP90, 0, 2.8), rawValue: `${keyPassesP90.toFixed(2)}/90` },
...
describe('Player Comparison Radar Profile Determination (Unit Tests)', () => {
  ...
  it('Test 5: should evaluate both players on identical profile and benchmark scale regardless of their other positions', () => {
    const playerA_positions = ['CAM', 'CM'];
    const playerB_positions = ['CAM', 'ST'];
  ...
  // Test 6 — Position to Radar Profile Mapping Coverage
  it('Test 6: should map all standard positions correctly into 7 Radar Profiles', () => {
    expect(getRadarProfile('GK')).toBe('GK');

    expect(getRadarProfile('CB')).toBe('DEF');
    expect(getRadarProfile('LB')).toBe('DEF');
    expect(getRadarProfile('RB')).toBe('DEF');
    expect(getRadarProfile('LWB')).toBe('DEF');
    expect(getRadarProfile('RWB')).toBe('DEF');
    ...
    expect(getRadarProfile('LM')).toBe('LM_RM');
    expect(getRadarProfile('RM')).toBe('LM_RM');
