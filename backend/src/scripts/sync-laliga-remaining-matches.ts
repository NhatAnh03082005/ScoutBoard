...
async function main() {
  ...
  console.log(
    '=== SCOUTBOARD: SYNC REMAINING LA LIGA MATCH STATS & AGGREGATE     ===',
  );
  console.log(
    '======================================================================\n',
  );
  ...
  console.log(
    `[API-Football Quota] Current: ${initialQuota.current}/${initialQuota.limit} requests | Remaining: ${initialQuota.remaining}`,
  );

  const safetyBuffer = 3;
  const usableRequests = Math.max(0, initialQuota.remaining - safetyBuffer);

  if (usableRequests <= 0) {
    console.warn(
      `⚠️ Remaining quota (${initialQuota.remaining}) is too close to limit. Stopping safely.`,
    );
    await app.close();
    return;
  }
  ...
  console.log(
    `Available budget: ${usableRequests} matches (keeping ${safetyBuffer} requests as safety buffer).\n`,
  );

  // 2. Query target finished matches without stats
  const candidateMatches: Array<{
    ...
    ORDER BY 
      CASE 
        ...
        WHEN (ht.name IN ('Atletico Madrid', 'Athletic Club', 'Villarreal', 'Real Sociedad', 'Girona', 'Real Betis', 'Sevilla', 'Valencia') 
           OR at.name IN ('Atletico Madrid', 'Athletic Club', 'Villarreal', 'Real Sociedad', 'Girona', 'Real Betis', 'Sevilla', 'Valencia')) THEN 3
        ELSE 4
      END ASC,
      m.match_date ASC
    LIMIT $1
  ...
  if (candidateMatches.length === 0) {
    console.log(
      '✓ All finished La Liga matches already have player statistics!',
    );
    await app.close();
    return;
  }
  ...
  candidateMatches.slice(0, 5).forEach((m, idx) => {
    console.log(
      `  ${idx + 1}. [${new Date(m.match_date).toISOString().slice(0, 10)}] ${m.home} vs ${m.away} (ID: ${m.external_id})`,
    );
  });
  ...
  console.log(
    '\nStarting API synchronization (rate limited at ~6.2s per request)...\n',
  );
  ...
  for (let i = 0; i < candidateMatches.length; i++) {
    ...
    process.stdout.write(
      `[${matchNum}/${candidateMatches.length}] Syncing fixture ${match.external_id} (${match.home} vs ${match.away}, ${dateStr})... `,
    );
    ...
    try {
      ...
      console.log(
        `✓ OK (${result.persistedCount} player stats, ${result.skippedCount} skipped)`,
      );
    } catch (err: any) {
      ...
      if (
        ...
        console.warn(
          '⚠️ Quota limit detected. Stopping match sync loop gracefully.',
        );
  ...
  console.log(
    `✓ Finished Match Sync: ${successCount}/${candidateMatches.length} fixtures synced.`,
  );
  console.log(
    `✓ Total player match statistic records persisted: ${totalPlayersPersisted}`,
  );
  console.log(
    '----------------------------------------------------------------------\n',
  );
  ...
  if (candidateMatches.length > 0 && candidateMatches[0].season_id) {
    ...
    console.log(
      '>>> Aggregating Season Statistics & Per-90 metrics for La Liga 2024-2025...',
    );
    try {
      ...
      console.log(
        `✓ Aggregation completed: ${aggResult.totalAggregated} players now have updated season stats & Per-90 benchmarks!\n`,
      );
    } catch (aggErr: any) {
  ...
  console.log(
    `=== SYNC COMPLETE! Final Quota: ${finalQuota.current}/${finalQuota.limit} requests | Remaining: ${finalQuota.remaining} ===`,
  );
  console.log(
    '======================================================================\n',
  );
