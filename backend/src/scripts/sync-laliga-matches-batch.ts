...
async function main() {
  ...
  console.log(
    '=== SCOUTBOARD: BATCH SYNC LA LIGA MATCHES & UPDATE PLAYER RADAR   ===',
  );
  console.log(
    '======================================================================\n',
  );
  ...
  if (initialQuota.remaining <= 0) {
    console.error(
      '❌ Daily quota is already exhausted. Please try again tomorrow.',
    );
    await app.close();
    return;
  }

  // Safety buffer: keep 2-3 requests reserve so we do not hard crash if quota boundary is tight
  const maxToFetch = Math.min(95, initialQuota.remaining);
  console.log(
    `🎯 Target: Fetching up to ${maxToFetch} high-priority La Liga matches.\n`,
  );

  // 2. Query target matches prioritized by top teams & match dates
  ...
  if (candidateMatches.length === 0) {
    console.log(
      '✅ No matches need syncing. All finished La Liga matches already have stats!',
    );
    await app.close();
    return;
  }
  ...
  console.log(
    '\n⏳ Starting synchronization (rate limited at ~6.5s per request to respect API limits)...\n',
  );
  ...
  for (let i = 0; i < candidateMatches.length; i++) {
    ...
    try {
      ...
      if (
        ...
        console.warn('⚠️ Quota limit reached. Stopping batch loop gracefully.');
  ...
  if (candidateMatches.length > 0 && candidateMatches[0].season_id) {
    ...
    console.log(
      '📊 Aggregating Season Statistics & Per-90 Radar Metrics for La Liga 2024-2025...',
    );
    try {
